...
class HttpContext {
 public:
  ...
  bool ParaseRequest(const char* data, int len);  // 解析请求
  bool IsCompleteRequest();  // 请求是否完成
  HttpRequest* GetRequest();  // 获取请求
  void ResetContextStatus();  // 重置状态

private:
  std::unique_ptr<HttpRequest> request_;  // 请求
  HttpRequestParaseState state_;  // 状态
};